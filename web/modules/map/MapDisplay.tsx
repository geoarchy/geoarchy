...
class MapDisplay extends React.Component<MapDisplayProps, MapDisplayState> {
    ...
    forLayerInGroups(layerGroups, fn, layerFilter = null) {
        if (layerFilter) {
            layerGroups.forEach(layerGroup => {
                layerGroup.layers &&
                    layerGroup.layers.filter(layerFilter).forEach(fn)
            })
            return
        }
    ...
    initializeMap() {
        ...
        if (isSupported) {
            this.map = new this.mapbox.Map({
                ...
                zoom: 9,
                style:
                    'mapbox://styles/rerooting2040/cjopaywcn4lbq2sn985cr5fcs',
    ...
    renderMapboxControlPortals(props) {
        ...
        return (
            ...
            ['bottom-left', 'bottom-right', 'top-right', 'top-left'].map(
                region => {
                    const regionCs = props.map.components.filter(
                        c => c.region === region
                    )
                    ...
                    return (
                        <div
                            key={`region-${region}`}
                            className={`mapboxgl-ctrl-${region}`}
                        >
                            {regionCs.map((C, n) => {
